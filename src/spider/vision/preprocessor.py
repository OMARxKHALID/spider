...
class Preprocessor:
    ...
    def process_image(image_bytes: bytes) -> np.ndarray:
        ...
        logger.info("[Phase 3/5] Pre-Processing: %dx%d (Sharpness: %.2f)", w, h, laplacian_var)
            
        # 1. Grayscale
        gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
        
        # Quality gate: skip heavy processing if already clean
        ...
        # 3. Dark-mode inversion (mean pixel > 128 -> likely light-on-dark)
        mean_val = gray.mean()
        if mean_val > 128:
    ...
    def deskew(image: np.ndarray) -> np.ndarray:
        # Simple deskew using minAreaRect on inverted image (foreground must be white for contours)
        ...
        # Use white border mode since text is black on white
